...
- 下木椅一次最多下 **3 張** 
- 下箏架或是調整箏架一次最多 **2 副** 
- 分配完下箏 `left` 後，先分配下一場表演者自己上或調整自己的箏架、木椅 `middle`，然後分配上箏 `right`，最後分配剩下的工作 `middle` 例如下架、下木椅
- 如果缺少人力，則輸出【空缺】[工作名稱]
- `middle` 與 `right` 兩區人員互斥（已指派 middle 者不進 right，反之亦然）
- `middle_heavy_workers`：已在中台做過工作的人，優先避免讓他們再去搬箏

**`pick_for_up_guzheng` 選人邏輯：**
- 必須排除 `blocked_for_right`（下場表演者已經要自己上或調整箏架、木椅）。
- 依池優先序選人：control → core → next_helpers → mic，找到即立即回傳。
...
| `apply_vacancy_color` | 將儲存格中含【空缺】的行標記為紅字，其餘行維持預設樣式。|
